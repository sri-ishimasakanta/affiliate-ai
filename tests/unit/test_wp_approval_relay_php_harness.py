...
C8.8 で PHP CLI をローカルに用意した (winget PHP.PHP.8.2 / php.net 公式 zip)。
...
_HARNESS = _ROOT / "wordpress" / "mu-plugins" / "bizfluxlab-approval-relay" / "tests" / "run.php"
...
def test_php_harness_passes() -> None:
    ...
    lint = subprocess.run([php, "-l", str(_HARNESS)], capture_output=True, text=True, timeout=60)
    assert lint.returncode == 0, lint.stdout + lint.stderr

    result = subprocess.run([php, str(_HARNESS)], capture_output=True, text=True, timeout=120)
...
def test_php_lints_all_runtime_files() -> None:
    ...
    for rel in (
        "bizfluxlab-approval-relay.php",
        "bizfluxlab-approval-relay/lib-core.php",
        "bizfluxlab-approval-relay/tests/run.php",
    ):
        r = subprocess.run([php, "-l", str(base / rel)], capture_output=True, text=True, timeout=60)
