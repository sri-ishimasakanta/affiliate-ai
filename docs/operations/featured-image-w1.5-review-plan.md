...
`planned` のまま (適用したら `applied` にする。§2 の 11)。**W1.5H (2026-09-26) で 21 記事とも
適用して確かめ、`applied` にした** (記録は `featured-image-pipeline.md` §7)。

| バッチ | 記事 | 画像の承認 (人) | WordPress への適用 |
| --- | --- | --- | --- |
| 1 | 7 / 2 / 6 / 8 / 9 | **承認済み** (W1.5B のあと、人の報告) | **適用済み** (W1.5H) |
| 2 | 3 / 12 / 13 / 15 | **承認済み** (W1.5C のあと、人の報告)。rose `#DB2777` もプロジェクト・タスク管理の色として承認 | **適用済み** (W1.5H) |
| 3 | 1 / 4 / 5 / 14 | **承認済み** (W1.5D のあと、人の報告)。orange `#EA580C` (CRM・SFA) も目で見て承認 | **適用済み** (W1.5H) |
| 4 | 10 / 11 / 16 / 17 / 18 | **承認済み** (W1.5E のあと、人の報告)。green `#16A34A` (RPA・自動化) も承認 | **適用済み** (W1.5H) |
| 5 | 19 / 21 / 22 | **承認済み** (W1.5F のあと、人の報告)。indigo `#4F46E5`・slate `#475569` も承認 | **適用済み** (W1.5H) |
...
## 6. 本番適用の計画 (W1.5G で計画、W1.5H で適用済み)

5 バッチ・21 枚とも人が目で見て承認した (§1.1)。W1.5G では WordPress を **読むだけ** で
対象を決め、適用の manifest を作った (その時点では書き込み 0)。**W1.5H (2026-09-26
01:15〜01:22 JST) で、人の承認を受けて 21 記事に 1 記事ずつ適用し、すべて確かめた**
(結果は `featured-image-pipeline.md` §7)。
